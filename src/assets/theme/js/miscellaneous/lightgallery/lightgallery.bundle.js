...
(function(factory) {
  ...
  JustifiedGallery.prototype.prepareBuildingRow = function(isLastRow) {
    ...
    if (isLastRow && this.settings.lastRow === 'hide' && !justifiable) {
      for (i = 0; i < this.buildingRow.entriesBuff.length; i++) {
        ...
        if (this.settings.cssAnimation) $entry.removeClass('entry-visible');
        else {
  ...
  JustifiedGallery.prototype.flushRow = function(isLastRow) {
    ...
    for (i = 0; i <= lastEntryIdx; i++) {
      $entry = this.buildingRow.entriesBuff[
    ...
    for (var i = 0; i < suffixRanges.length; i++) {
      if ($.type(suffixRanges[i]) === 'string') {
        try {
          ...
          newSizeRngSuffixes[numIdx] = this.settings.sizeRangeSuffixes[
            suffixRanges[i]
          ];
        } catch (e) {
        ...
        newSizeRngSuffixes[suffixRanges[i]] = this.settings.sizeRangeSuffixes[
          suffixRanges[i]
        ];
      }
    }
